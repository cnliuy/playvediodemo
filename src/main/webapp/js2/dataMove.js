...
var MultiMove = {
	 ...
	 *            size 页面展示数据长度，默认为1
	 * @param {Number}
	 *            rowNum 多行多列时，表示几行默认2行，当isMultSplit为true时，必传（可选）
	 * @param {Number}
    ...
    initializ : function(_config) {
		...
		this.rowNum= _config.rowNum || 2;
        this.splitNum = _config.colNum || parseInt(this.size / this.rowNum);
        ...
        this.moveFlag = Boolean(this.focusId != "" && this.focusTop != 0 && this.focusStep != 0);
        
        this.addPage = _config.addPage || 0;
    },
	/**
    ...
    setFocus : function() {
        ...
        if(this.moveFlag) {
            if(!this.isFocusFix){           
                ...
                if(this.focusLeft && this.focusTop){  
                	this.left = (this.focusIndex % this.splitNum) * this.focusLeftStep + this.focusLeft;
                	this.top = (this.curCol - 1) * this.focusStep + this.initFocusTop;                  
                	moveSplit({"element" : this.focusId, "destination" : {"left" : this.left, "top" : this.top }, "speed" : this.percent});
                }else{
    ...
    slideUpdown : function() {
		if(this.totalPage<2){
			$("srcoll").parentNode.style.display = "none";
			return;
		}else{
			$("srcoll").parentNode.style.display = "block";
		}
		
		var lineH=parseInt(getCurrentStyle($("srcoll").parentNode,"height"),10);
		$("srcoll").style.height=parseInt(lineH/this.totalPage,10)+"px";
		var scrollBarH=parseInt(getCurrentStyle($("srcoll"),"height"),10);
		var scrollBarTop=((lineH-scrollBarH)/(this.totalPage-1))*(this.curPage-1);
		$("srcoll").style.top=scrollBarTop+"px";
...
var List = {
    ...
    initListData : function(_data, _curPage, _totalPage){
        ...
        if(this.isMultSplit) {
			...
			this.focusTop = this.finalFocusTop + (this.curCol - 1) * this.focusStep;//add by honggj
        }
    },
	/**
	...
	/**
	 * @description moveList 单列列表数据移动函数
	 */     
        ...
        if(1 < this.totalPage){
            ...
            if(this.focusIndex < 0) {
				...
				if(this.isMultSplit && this.curCol > 1){
					this.focusTop = this.initFocusTop + this.initFocusStep;
				}
            }
            if(this.focusIndex > this.length - 1) {
            	this.focusIndex = 0;
            	if(this.isMultSplit){
            		this.focusTop = this.initFocusTop;
            	}
            }
        }      
    ...
    changeCol : function(offset) {
        if(this.isMultSplit) {
        	this.curCol = Math.ceil((this.focusIndex + 1) / this.splitNum);
        }
    },
	/**
    ...
    setPageInfo : function(_pageId) {
        ...
        $(_pageId).innerHTML = "第" + (this.curPage + this.addPage) + "/" + (this.totalPage + this.addPage) + "页";
    },
	/**
	 * @description initListArrow 列表页面初始化箭头函数
	 */        
    initListArrow : function(_upSrcName, _downSrcName){
        if(this.length > 0){
            $(this.arrowDR).style.visibility = "visible";
            $(this.arrowUL).style.visibility = "visible";
            this.setListArrow(_upSrcName, _downSrcName);
        }else{
            $(this.arrowDR).style.visibility = "hidden";
            $(this.arrowUL).style.visibility = "hidden";         
        }
...
function moveSplit(data){	
    ...
    addEventHandler(document, "keydown", shileEvent, false);    
    moveTimer = window.setInterval(function() {
        ...
        if (startPosition.left == data.destination.left && startPosition.top == data.destination.top) {
            ...
            removeEventHandler(document, "keydown", shileEvent);
        }
    }, 10);
}
